<template>
  <div v-loading.fullscreen.lock="saveLoading" element-loading-spinner="el-icon-loading" :element-loading-text="$t('operation.processing')" class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.code" :placeholder="$t('commonUse.編碼')" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
      <el-input v-model="listQuery.name" :placeholder="$t('commonUse.名稱')" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column :label="$t('commonUse.編碼')" min-width="120" align="center" sortable="custom" prop="code">
        <template slot-scope="{row}">
          <span>{{ row.code }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('commonUse.名稱')" min-width="120" align="center">
        <template slot-scope="{row}">
          <span>{{ row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('tenant.語言')" min-width="100" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column :label="$t('tenant.手機號')" min-width="120" align="center" sortable="custom" prop="phone">
        <template slot-scope="{row}">
          <span>{{ row.phone }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('tenant.郵箱')" min-width="120" align="center" sortable="custom" prop="email">
        <template slot-scope="{row}">
          <span>{{ row.email }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('tenant.採用獨立數據庫')" min-width="80" align="center" sortable="custom" prop="isStandaloneDatabase">
        <template slot-scope="{row}">
          <span>{{ row.isStandaloneDatabase | isStandaloneDatabaseFilter }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('tenant.數據庫連接配置名稱')" min-width="100" align="center" sortable="custom" prop="defaultConnectionStringName">
        <template slot-scope="{row}">
      ...
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="200px" style="">
        <el-row>
          <el-col :span="24">
            <el-row>
              <el-col :span="12">
                <el-form-item :label="$t('commonUse.編碼')" prop="code">
                  <el-input v-model.trim="temp.code" :placeholder="$t('commonUse.不輸入自動生成')" :readonly="dialogStatus!='create'" />
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item :label="$t('commonUse.名稱')" prop="name">
                  <el-input v-model.trim="temp.name" :placeholder="$t('tenant.請輸入名稱')" />
                </el-form-item>
              </el-col>
            </el-row>
            <el-col :span="12">
              <el-form-item :label="$t('tenant.手機號')" prop="phone">
                <el-input v-model.trim="temp.phone" :placeholder="$t('tenant.請輸入手機號')" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item :label="$t('tenant.郵箱')" prop="email">
                <el-input v-model.trim="temp.email" :placeholder="$t('tenant.請輸入郵箱')" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item :label="$t('tenant.採用獨立數據庫')">
            ...
            <el-col v-if="!temp.isStandaloneDatabase" :span="12">
              <el-form-item :label="$t('tenant.數據庫連接配置名稱')" prop="defaultConnectionStringName">
            ...
            <el-col v-if="temp.isStandaloneDatabase" :span="24">
              <el-form-item :label="$t('tenant.獨立的數據庫連接字符串')" prop="standaloneDatabaseConnectionString">
                <el-input v-model.trim="temp.standaloneDatabaseConnectionString" :placeholder="$t('tenant.請輸入獨立的數據庫連接字符串')" />
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item :label="$t('tenant.語言')" prop="language">
            ...
            <el-col :span="24">
              <el-form-item :label="$t('tenant.描述')">
                <el-input v-model.trim="temp.memo" type="textarea" :rows="4" :placeholder="$t('tenant.請輸入描述')" />
              </el-form-item>
            </el-col>
          </el-col>
        </el-row>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        name: [{ required: true, message: this.$t('tenant.名稱必填'), trigger: 'change' }],
        email: [{ type: 'email', message: this.$t('tenant.請輸入正確的郵箱地址'), trigger: ['blur', 'change'] }],
        language: [{ required: true, message: this.$t('tenant.語言必選'), trigger: 'change' }]
      },
