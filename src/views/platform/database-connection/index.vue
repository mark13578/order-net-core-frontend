<template>
  <div v-loading.fullscreen.lock="saveLoading" element-loading-spinner="el-icon-loading" :element-loading-text="$t('operation.processing')" class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('commonUse.名稱')" min-width="150" align="center">
        <template slot-scope="{row}">
          <span>{{ row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('databaseConnection.數據庫驅動類型')" min-width="150" align="center" sortable="custom" prop="databaseProviderType">
        <template slot-scope="{row}">
          <span>{{ row.databaseProviderType | databaseProviderTypeFilter }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('databaseConnection.數據庫連接字符串')" min-width="150" align="center" sortable="custom" prop="connectionString">
        <template slot-scope="{row}">
      ...
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="200px" style="">
        <el-row>
          <el-col :span="24">
            <el-row>
              <el-col :span="12">
                <el-form-item :label="$t('commonUse.名稱')" prop="name">
                  <el-input v-model.trim="temp.name" :placeholder="$t('databaseConnection.請輸入名稱')" :readonly="dialogStatus!='create'" />
                </el-form-item>
              </el-col>
            </el-row>
            <el-col :span="12">
              <el-form-item :label="$t('databaseConnection.數據庫驅動類型')">
            ...
            <el-col :span="24">
              <el-form-item :label="$t('databaseConnection.數據庫連接字符串')" prop="connectionString">
                <el-input v-model.trim="temp.connectionString" :placeholder="$t('databaseConnection.請輸入數據庫連接字符串')" />
              </el-form-item>
            </el-col>
            <el-col :span="24">
              <el-form-item :label="$t('databaseConnection.指定適用租户範圍')" prop="rangeTenantIds">
                <tenant-multiple-select v-model="temp.rangeTenantIds" :placeholder="$t('databaseConnection.默認支持全部租户')" />
              </el-form-item>
            </el-col>
            <el-col :span="24">
              <el-form-item :label="$t('databaseConnection.描述')">
                <el-input v-model="temp.memo" type="textarea" :rows="4" :placeholder="$t('databaseConnection.請輸入描述')" />
              </el-form-item>
            </el-col>
          </el-col>
        </el-row>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        name: [{ required: true, message: this.$t('databaseConnection.名稱必填'), trigger: 'change' }]
      },
